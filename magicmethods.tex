...
\item[\code{__concat__(self, other)}]
Lastly, you can define behavior for concatenating your sequence with another by defining \code{__concat__}. It should return a new sequence constructed from \code{self} and \code{other}. \code{__concat__} is invoked with the \code{+} operator when it is called on two sequences.  
...
\hline
\code{__concat__(self, value)} & \code{self + other} & Concatenation of two sequences\\
\hline
